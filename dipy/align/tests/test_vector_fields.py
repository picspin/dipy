...
def test_random_displacement_field_2d(rng):
    ...
    trans = np.array(
        [[1, 0, -t * from_shape[0]], [0, 1, -t * from_shape[1]], [0, 0, 1]]
    )
    ...
    for theta in [-1 * np.pi / 6.0, 0.0, np.pi / 5.0]:  # rotation angle
        for s in [0.83, 1.3, 2.07]:  # scale
            ...
            field, assignment = vfu.create_random_displacement_2d(
                np.array(from_shape, dtype=np.int32),
                from_grid2world,
            ...
            for k in range(nslices):
                for i in range(nrows):
                    for j in range(ncols):
                        ...
                        jj = j - mid_col
                        theta = np.arctan2(ii, jj)
                        expected_d[k, i, j, 0] = kk * (
                            1.0 / (1 + b * np.cos(m * theta)) - 1.0
                        )
                        expected_d[k, i, j, 1] = ii * (
                            1.0 / (1 + b * np.cos(m * theta)) - 1.0
                        )
                        expected_d[k, i, j, 2] = jj * (
                            1.0 / (1 + b * np.cos(m * theta)) - 1.0
                        )
...
def test_sphere():
    ...
    _O = np.ones(sh)
    X[0, ...] = x_0[:, None, None] * _O - cs
    X[1, ...] = x_1[None, :, None] * _O - cr
    X[2, ...] = x_2[None, None, :] * _O - cc
    nrm = np.sqrt(np.sum(X**2, axis=0))
    for radius in [0, 7, 17, 32]:
        expected = nrm <= radius
...
def test_compose_vector_fields_2d(rng):
    ...
    for time_scaling in [0.25, 0.5, 1.0, 2.0, 4.0]:
        composition, stats = vfu.compose_vector_fields_2d(
            disp1, disp2 / time_scaling, premult_index, premult_disp, time_scaling, None
        )
...
def test_invert_vector_field_2d():
    ...
    for theta in [-1 * np.pi / 5.0, 0.0, np.pi / 5.0]:  # rotation angle
        for s in [0.5, 1.0, 2.0]:  # scale
            ...
            inv_approx = vfu.invert_vector_field_fixed_point_2d(
...
def test_gradient_2d(rng):
    ...
    assert_raises(
        ValueError, vfu.sparse_gradient, img, invalid_affine, img_spacing, sample
    )
    assert_raises(
        ValueError, vfu.sparse_gradient, img, sp_to_grid, invalid_spacings, sample
    )
    ...
    # Verify exception is raised when passing invalid affine or spacings
    assert_raises(ValueError, vfu.gradient, img, invalid_affine, img_spacing, sh, T)
    assert_raises(
        ValueError, vfu.gradient, img, sp_to_grid, img_spacing, sh, invalid_affine
    )
    assert_raises(ValueError, vfu.gradient, img, sp_to_grid, invalid_spacings, sh, T)
...
def test_gradient_3d(rng):
    ...
    assert_equal(diff.max() < 1e-5, True)
    # Verify exception is raised when passing invalid affine or spacings
    assert_raises(ValueError, vfu.gradient, img, invalid_affine, img_spacing, shape, T)
    assert_raises(
